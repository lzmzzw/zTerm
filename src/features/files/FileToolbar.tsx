...
export function FileToolbar({
  ...
  return (
    <div className="zt-file-toolbar">
      ...
      <button type="button" aria-label="上传" disabled={disabled || loading} onClick={() => void onUpload()}>
        <Upload size={14} aria-hidden="true" />
      </button>
      <button type="button" aria-label="下载" disabled={disabled || !hasSelection} onClick={() => void onDownload()}>
        <Download size={14} aria-hidden="true" />
      </button>
      <button type="button" aria-label="重命名" disabled={disabled || !hasSelection} onClick={() => void onRename()}>
