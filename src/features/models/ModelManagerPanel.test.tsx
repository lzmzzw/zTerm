...
describe("ModelManagerPanel", () => {
  it("keeps the left panel as a model list and creates/tests a draft model in a centered dialog", async () => {
    const onSaveProvider = vi.fn().mockResolvedValue(undefined);
    const onTestProviderDraft = vi.fn().mockResolvedValue({
      ok: true,
      message: "模型测试通过",
      output: "pong from model",
    });
    ...
    const view = render(
      <ModelManagerPanel
        ...
        onTestProviderDraft={onTestProviderDraft}
      />,
    );
    ...
    await click(button(dialog, "测试模型"));
    expect(dialog.textContent).toContain("pong from model");
    ...
    expect(onTestProviderDraft).toHaveBeenCalledWith({
      prompt: "say pong",
  ...
  it("shows structured backend errors when draft testing fails", async () => {
    const view = render(
      <ModelManagerPanel
        providers={[]}
        loading={false}
        ...
        onDeleteProvider={vi.fn()}
        onTestProviderDraft={vi.fn().mockRejectedValue({
          code: "ai",
